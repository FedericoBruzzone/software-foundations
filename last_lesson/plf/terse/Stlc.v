...
Inductive tm : Type :=
  | tm_var   : string -> tm
  | tm_app   : tm -> tm -> tm (* (lambda (x : Bool): x)True *)
...
(* QUIZ
    ...
    (2) [exists T,  empty |-- \y:Bool->Bool, \x:Bool, y x \in T]

    (3) [exists T,  empty |-- \y:Bool->Bool, \x:Bool, x y \in T]

    (4) [exists S, x:S |-- \y:Bool->Bool, y x \in (Bool->Bool)->S]
...
(* QUIZ
    ...
    (1) [exists T,  empty |-- \y:Bool->Bool->Bool, \x:Bool, y x \in T]

    (2) [exists S T,  x:S |-- x x x \in T]

    (3) [exists S U T,  x:S, y:U |-- \z:Bool, x (y z) \in T]

    (4) [exists S T,  x:S |-- \y:Bool, x (x y) \in T]
